...
This is the sensitive multivariate complement to the whole-brain GLM+TFCE
(gvs_glm_tfce.py), which found no univariate GVS effect anywhere. A spherical
...
session we obtain a whole-brain decoding-accuracy map (chance = 0.5, ROC AUC).

Group inference: the per-subject (accuracy - 0.5) maps are tested with sign-flip
TFCE permutation (FWE-corrected) via non_parametric_inference -- within OFF,
within ON, and paired ON-OFF -- exactly as in gvs_glm_tfce.py.
...
envelope 0/10/16/20 within each carrier.
"""
...
DEFAULT_OUT_DIR = ROOT / "figures" / "GVS_effects" / "searchlight_mvpa"
...
def _load_session_trials(run_specs, session_runs, n_vox, trials_per_condition):
...
def _labels_for_target(stim: np.ndarray, target: str):
    """Return (y, keep_mask) for the decoding target; -1 in y marks dropped trials."""
    if target == "stim":
        y = (stim != SHAM_STIM).astype(np.int8)  # 0 sham, 1 active
...
def _to_img(flat: np.ndarray, mask_flat, shape, affine, fill=0.0):
...
def main() -> None:
    ...
    ap.add_argument("--time-one", action="store_true", help="run one session searchlight and report timing, then exit")
    args = ap.parse_args()
    ...
    for idx, key in enumerate(session_keys, start=1):
        ...
        if len({s.run for s in runs}) < 2:
            print(f"skip {subject} ses-{session}: <2 runs")
            continue
        ...
        if cache_path.exists() and not args.time_one:
            ...
            print(f"[{idx}/{len(session_keys)}] cached {subject} ses-{session}")
            continue

        X, stim, run = _load_session_trials(runs, runs, n_vox, args.trials_per_condition)
        y, keep = _labels_for_target(stim, args.target)
        X, y, run = X[keep], y[keep], run[keep]

        # mask: finite & non-constant voxels in this session
        ...
        t0 = time.time()
        sl = SearchLight(
            mask_img=mask_img, process_mask_img=mask_img, radius=args.radius_mm,
            estimator=estimator, scoring="roc_auc", cv=LeaveOneGroupOut(), n_jobs=args.n_jobs, verbose=0,
        )
        sl.fit(img4d, y, groups=run)
        dt = time.time() - t0
        acc = np.asarray(sl.scores_, dtype=np.float32).ravel()
        acc_maps[key] = acc
        cache_mask = mask_flat if cache_mask is None else cache_mask
        ...
        print(f"[{idx}/{len(session_keys)}] {subject} ses-{session}: "
              f"n=({n0},{n1}) meanAUC={acc[mask_flat].mean():.3f} maxAUC={acc[mask_flat].max():.3f} {dt:.0f}s")
        if args.time_one:
            print(f"\nONE-SESSION TIMING: {dt:.1f}s for {int(mask_flat.sum())} voxels. "
                  f"Full run ~= {dt*len(session_keys)/60:.0f} min for {len(session_keys)} sessions (x targets).")
            pd.DataFrame(rows).to_csv(args.out_dir / f"timing_{args.target}.csv", index=False)
            return

    pd.DataFrame(rows).to_csv(args.out_dir / f"acc_summary_{args.target}.csv", index=False)

    # ---- group TFCE inference on (AUC - chance) ----
    from nilearn.glm.second_level import non_parametric_inference

    # common mask across sessions
    ...
    analyses = {
        "OFF": list(off.values()),
        "ON": list(on.values()),
        "interaction_ON_minus_OFF": [on[s] - off[s] + CHANCE for s in sorted(set(off) & set(on))],
    }

    summary_rows = []
    for label, maps in analyses.items():
        if len(maps) < 5:
            print(f"skip {label}: {len(maps)} subjects"); continue
        # one-sample test of (AUC - chance); for interaction the +CHANCE keeps the same code path (mean-0 test)
        imgs = [_to_img(m - CHANCE, finite_all, shape, affine) for m in maps]
        design = pd.DataFrame({"intercept": np.ones(len(imgs))})
        print(f"{args.target}/{label}: n={len(imgs)}, TFCE perms={args.n_perm}")
        ...
        grp_mean_auc = float(np.mean([m[finite_all].mean() for m in maps])) if not label.startswith("inter") else float("nan")
        summary_rows.append({"target": args.target, "analysis": label, "n_subjects": len(imgs),
                             "group_mean_auc": grp_mean_auc, "n_sig_voxels_fwe05": n_sig, "min_fwe_p": min_p})
        print(f"  -> mean AUC={grp_mean_auc:.3f}, sig voxels(FWE<.05)={n_sig}, min FWE p={min_p:.4g}")

    summary = pd.DataFrame(summary_rows)
    summary.to_csv(args.out_dir / f"tfce_summary_{args.target}.csv", index=False)
    print("\n=== Searchlight MVPA TFCE summary ===")
