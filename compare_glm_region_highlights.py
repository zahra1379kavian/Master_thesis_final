#!/usr/bin/env python3
"""Compare atlas-region emphasis across GLM, GLMsingle, and weight maps."""
...
def _load_img(path: Path) -> nib.Nifti1Image:
    if not path.exists():
        raise FileNotFoundError(f"Missing input map: {path}")
    return nib.load(str(path))


def _check_same_grid(reference: nib.Nifti1Image, images: dict[str, nib.Nifti1Image]) -> None:
...
def _build_analysis_mask(map_data: dict[str, np.ndarray]) -> np.ndarray:
    """Use the nonzero GLMsingle/weight support to avoid standard-GLM full-FOV spillover."""
...
def _top_weight_mask(weights: np.ndarray, analysis_mask: np.ndarray, n_voxels: int) -> tuple[np.ndarray, float]:
...
def _make_masks(
    ...
    for method in z_methods:
        ...
        masks.extend(
            [
                MaskSpec(
                    method=method,
                    mask_name=f"{method} positive z",
                    family="positive_z",
                    threshold_definition=f"z >= {z_threshold:g}",
                    threshold_value=float(z_threshold),
                    values=values,
                    mask=finite & (values >= z_threshold),
                ),
                MaskSpec(
                    method=method,
                    mask_name=f"{method} negative z",
                    family="negative_z",
                    threshold_definition=f"z <= -{z_threshold:g}",
                    threshold_value=float(-z_threshold),
                    values=values,
                ...
                MaskSpec(
                    method=method,
                    mask_name=f"{method} absolute z",
                    family="absolute_z",
                    threshold_definition=f"|z| >= {z_threshold:g}",
                    threshold_value=float(z_threshold),
                    values=values,
                    mask=finite & (np.abs(values) >= z_threshold),
                ),
            ]
        )
    ...
    for weight_percentile in weight_percentiles:
        ...
        masks.append(
            MaskSpec(
                method="Optimization weights",
                mask_name=f"Optimization p{weight_percentile:g}",
                family="weight_percentile",
                threshold_definition=f"top {100.0 - weight_percentile:g}% of nonzero weights",
                threshold_value=weight_threshold,
                values=weights,
                mask=finite_weights & (weights >= weight_threshold),
            )
        )
    ...
    for source in [mask for mask in masks if mask.family in {"positive_z", "absolute_z"}]:
        ...
        masks.append(
            MaskSpec(
                method="Optimization weights",
                mask_name=f"Optimization matched to {source.method} {source.family.replace('_', ' ')}",
                family=f"weight_matched_{source.family}",
                threshold_definition=f"top {n_voxels:,} weights; voxel-count matched to {source.method} {source.family.replace('_', ' ')}",
                threshold_value=matched_threshold,
                values=weights,
                mask=matched_mask,
            )
        )
...
def _region_denominators(groups: list[ROIGroup], analysis_mask: np.ndarray) -> dict[str, int]:
...
def _assign_regions(
    ...
    if selected_ijk.size == 0:
        return pd.DataFrame(
            [
                {
                    "method": mask_spec.method,
                    "mask_name": mask_spec.mask_name,
                    "family": mask_spec.family,
                    "threshold_definition": mask_spec.threshold_definition,
                    "threshold_value": mask_spec.threshold_value,
                    "roi_name": UNASSIGNED_ROI,
                    "hemisphere": "NA",
                    "n_voxels": 0,
                    "percent_of_map": np.nan,
                    "percent_of_assigned_map": np.nan,
                    "atlas_region_voxels": np.nan,
                    "percent_of_atlas_region": np.nan,
                    "present_for_report": False,
                    "mean_map_value": np.nan,
                    "min_map_value": np.nan,
                    "max_map_value": np.nan,
                    "mean_abs_value": np.nan,
                    "peak_abs_value": np.nan,
                    "x_mm": np.nan,
                    "y_mm": np.nan,
                    "z_mm": np.nan,
                    "source": "No selected voxels",
                }
            ]
        )
        ...
        rows.append(
            {
                ...
                "family": mask_spec.family,
                "threshold_definition": mask_spec.threshold_definition,
                "threshold_value": float(mask_spec.threshold_value),
                "roi_name": roi_name,
                "hemisphere": "NA" if roi_name == UNASSIGNED_ROI else BILATERAL_HEMISPHERE_LABEL,
                "n_voxels": int(positions.size),
                "percent_of_map": float(positions.size / total_voxels) if total_voxels else np.nan,
                "percent_of_assigned_map": (
                    float(positions.size / assigned_voxels)
                    if assigned_voxels and roi_name != UNASSIGNED_ROI
                    else np.nan
                ),
                "atlas_region_voxels": atlas_region_voxels,
                "percent_of_atlas_region": percent_of_atlas_region,
                "present_for_report": bool(positions.size >= min_report_voxels and roi_name != UNASSIGNED_ROI),
                "mean_map_value": float(np.mean(roi_values)),
                "min_map_value": float(np.min(roi_values)),
                "max_map_value": float(np.max(roi_values)),
                "mean_abs_value": float(np.mean(np.abs(roi_values))),
                "peak_abs_value": float(np.max(np.abs(roi_values))),
...
def _top_region_text(region_df: pd.DataFrame, mask_name: str, max_regions: int = 8) -> str:
    rows = (
        region_df[
            region_df["mask_name"].eq(mask_name)
            & region_df["present_for_report"]
...
def _summarize_masks(masks: list[MaskSpec], region_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for spec in masks:
        n_voxels = int(np.count_nonzero(spec.mask))
        unassigned = int(
            region_df.loc[
                region_df["mask_name"].eq(spec.mask_name) & region_df["roi_name"].eq(UNASSIGNED_ROI),
                "n_voxels",
            ].sum()
        )
        n_components, largest_component = _component_stats(spec.mask)
        reportable_regions = int(
            region_df.loc[
                region_df["mask_name"].eq(spec.mask_name) & region_df["present_for_report"],
                "roi_name",
            ].nunique()
        )
        rows.append(
            {
                ...
                "family": spec.family,
                "threshold_definition": spec.threshold_definition,
                "threshold_value": spec.threshold_value,
                "n_voxels": n_voxels,
                "n_unassigned_voxels": unassigned,
                "atlas_assigned_fraction": float(1.0 - unassigned / n_voxels) if n_voxels else np.nan,
                "n_components": n_components,
                "largest_component_voxels": largest_component,
                "n_reportable_regions": reportable_regions,
...
def _overlap_table(masks: list[MaskSpec], groups: list[ROIGroup], min_report_voxels: int) -> pd.DataFrame:
    rows = []
    for left, right in itertools.combinations(masks, 2):
        ...
        rows.append(
            {
                "mask_a": left.mask_name,
                "family_a": left.family,
                "mask_b": right.mask_name,
                "family_b": right.family,
                "n_voxels_a": left_n,
                "n_voxels_b": right_n,
                "shared_voxels": intersection,
                "dice": float(2 * intersection / (left_n + right_n)) if (left_n + right_n) else np.nan,
                "jaccard": float(intersection / union) if union else np.nan,
                "overlap_coefficient": float(intersection / min(left_n, right_n)) if min(left_n, right_n) else np.nan,
                "shared_top_regions": _intersection_top_regions(intersection_mask, groups, min_report_voxels),
            }
        )
    return pd.DataFrame(rows)


def _wide_region_table(region_df: pd.DataFrame, mask_names: list[str], value_col: str = "percent_of_map") -> pd.DataFrame:
    rows = region_df[
        region_df["mask_name"].isin(mask_names)
        & ~region_df["roi_name"].eq(UNASSIGNED_ROI)
    ].copy()
    return rows.pivot_table(index="roi_name", columns="mask_name", values=value_col, aggfunc="sum", fill_value=0.0)


def _plot_region_heatmap(
    region_df: pd.DataFrame,
    mask_names: list[str],
    out_base: Path,
    min_row_percent: float = DEFAULT_HEATMAP_MIN_ROW_PERCENT,
) -> None:
        ...
        for col in range(values.shape[1]):
            ...
            if value >= 2.0:
                ax.text(
                    ...
                    va="center",
                    fontsize=6.5,
                    color="white" if value > np.nanmax(values) * 0.45 else "black",
                )
...
def _short_mask_label(name: str) -> str:
    replacements = {
        "Standard GLM positive z": "Standard GLM",
        "GLMsingle Type A positive z": "GLMsingle A",
        "GLMsingle Type D positive z": "GLMsingle D",
        "Optimization p80": "Opt. p80",
        "Optimization p90": "Opt. p90",
    }
    return replacements.get(name, name.replace("Optimization matched to ", "Opt. matched\n"))


def _plot_overlap_heatmap(overlap_df: pd.DataFrame, mask_names: list[str], out_base: Path) -> None:
...
def _top_region_bullets(region_df: pd.DataFrame, mask_name: str, max_regions: int = 8) -> list[str]:
    rows = (
        region_df[
            region_df["mask_name"].eq(mask_name)
            & region_df["present_for_report"]
    ...
    return [
        f"{row.roi_name}: {int(row.n_voxels):,} voxels ({100.0 * float(row.percent_of_map):.1f}% of highlighted map)"
    ...
    for mask_name in primary_mask_names:
        ...
        primary_lines.append(
            "\n".join(
                [
                    f"### {mask_name}",
                    f"- Highlighted voxels: {int(row.n_voxels):,}",
                    f"- Atlas-assigned fraction: {float(row.atlas_assigned_fraction):.1%}",
                    f"- Reportable regions: {int(row.n_reportable_regions):,}",
                    "- Top regions: " + "; ".join(top_regions),
                ]
            )
        )

    primary_overlap = overlap_df[
        overlap_df["mask_a"].isin(primary_mask_names) & overlap_df["mask_b"].isin(primary_mask_names)
    ].copy()
    primary_overlap = primary_overlap.sort_values("dice", ascending=False)
    overlap_lines = [
        f"- {row.mask_a} vs {row.mask_b}: Dice={row.dice:.3f}, Jaccard={row.jaccard:.3f}, shared={int(row.shared_voxels):,}; {row.shared_top_regions or 'no shared reportable region'}"
...
def _write_json(
    ...
    payload = {
        "inputs": [{"method": spec.method, "path": str(spec.path), "kind": spec.kind} for spec in map_specs],
        "z_thresholds": {method: float(threshold) for method, threshold in z_thresholds.items()},
        "weight_percentiles": [float(percentile) for percentile in weight_percentiles],
        "min_report_voxels": int(min_report_voxels),
        "analysis_mask": {
            "definition": "union of nonzero GLMsingle Type A, GLMsingle Type D, and optimization-weight voxels",
            "n_voxels": int(np.count_nonzero(analysis_mask)),
        },
        "atlas": metadata,
    }
...
def main() -> None:
    ...
    map_specs = [
        MapSpec("Standard GLM", args.standard_glm, "z"),
        MapSpec("GLMsingle Type A", args.glmsingle_a, "z"),
        MapSpec("GLMsingle Type D", args.glmsingle_d, "z"),
        MapSpec("Optimization weights", args.weight_map, "weight"),
    ]
    ...
    z_thresholds = {
        "Standard GLM": float(args.standard_z_threshold),
        "GLMsingle Type A": float(args.glmsingle_z_threshold),
        "GLMsingle Type D": float(args.glmsingle_z_threshold),
    }
    ...
    primary_mask_names = [
        f"Standard GLM positive z",
        f"GLMsingle Type A positive z",
        f"GLMsingle Type D positive z",
    ] + [f"Optimization p{percentile:g}" for percentile in weight_percentiles]
    ...
    matched_mask_names = primary_mask_names + [
        mask.mask_name
        for mask in masks
        if mask.family == "weight_matched_positive_z"
    ]
    matched_overlap_df = _overlap_table(
        [mask for mask in masks if mask.mask_name in matched_mask_names],
        groups,
        args.min_report_voxels,
    )
    matched_overlap_df = matched_overlap_df[
        ~(
            matched_overlap_df["mask_a"].isin(primary_mask_names)
            & matched_overlap_df["mask_b"].isin(primary_mask_names)
        )
    ]
    overlap_df = pd.concat(
        [overlap_df.assign(overlap_set="primary"), matched_overlap_df.assign(overlap_set="count_matched")],
        ignore_index=True,
    ).drop_duplicates(subset=["overlap_set", "mask_a", "mask_b"])
    ...
    _plot_overlap_heatmap(
        overlap_df[overlap_df["overlap_set"].eq("primary")],
        primary_mask_names,
        out_base,
    )
    _write_report(
        ...
        region_df=region_df,
        overlap_df=overlap_df[overlap_df["overlap_set"].eq("primary")],
        primary_mask_names=primary_mask_names,
        metadata=metadata,
